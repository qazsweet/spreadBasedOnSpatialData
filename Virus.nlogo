...
to setup-environment
  ;;set raster-dataset gis:load-dataset "Data_exercise/land_ascii.asc"
  ;;gis:set-world-envelope (gis:envelope-of raster-dataset)
  ;;gis:paint raster-dataset 0
  ;;gis:apply-raster raster-dataset land_cover ;apply raster values to patches
  ;;ask patches with [land_cover = 2] [set pcolor blue]
  ;;ask patches with [land_cover = 1] [set pcolor yellow]
  set vector-dataset gis:load-dataset "Data_exercise/env_clip.shp"
...
to move
  ...
  while [m = False] [
    ...
    if patch-ahead 1 != nobody [
      if [land_cover] of (patch-ahead 1) = 1 [
        fd 1
        set m True
  ]
  ]
